...
###### Reminder: The rates of color of people are already multipied by 100, which are stands for percentages.
This graph shows the percentage of people of color in prison (calculated by dividing the number of people of color in prison by the total number of people of color, and already in percentage form). This graph shows the year-to-year trend in the percentage of people of color in prison. We can see that this ratio peaked in the late 1990s, but the overall trend is downward.
...
###### Reminder: The rates of color of people are already multipied by 100, which are stands for percentages.
This graph shows the proportion of white people in prison (calculated by dividing the number of white people in prison by the total number of white people, and already in percentage form). This graph shows the year-to-year trend in the proportion of white people in prison. We can see that this rate has increased year by year but has remained roughly the same since 2010.
...
The following diagram can clearly shows **the relationship between the two variables**
```{r, echo=FALSE, message=FALSE}
plot(p_chart)
```

###### Reminder: The rates of color of people are already multipied by 100, which are stands for percentages.
This figure clearly shows the relationship between the two variables. By comparison, it can be found that the rate of people of color entering prison is higher than that of white people every year, but the difference between the two rates is decreasing year by year. In other words, the impact of racial discrimination on justice is decreasing.

Here is also a graph just talking about the relationship between two variables.
```{r, echo=FALSE, message=FALSE}
plot(p_relationship)
```
...
Below is a map of the United States showing the color of people‘s jail population in every state. 
```{r, echo=FALSE, message=FALSE, warning = FALSE}
plot(p_map)
```
...
- The second the limitation of this file is that the color of people I explored is only AAPI, Black, Latinx, Native American race. Because this data frame lacks the total data of other race in total(even no data about the other race aged from 15-64), it is impossible to calcuate the rate by using `other race in jail` dividing by `other race in total` in order to get the rate.